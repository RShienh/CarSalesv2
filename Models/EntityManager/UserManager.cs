...
using System.Linq;
using System.Web;
using CarSale.Models.DB;
using CarSale.Models.ViewModel;
using System.Collections.Generic;

namespace CarSale.Models.EntityManager
{
    public class UserManager
    {
        ...
        public string GetUserPassword(string loginName)
        {
            ...
            {
                var user = entities.SYSUsers.Where(o => o.LoginName.ToLower().Equals(loginName));
                if (user.Any())
                {
            ...
            using (DBEntities db = new DBEntities())
            {
                using (var dbContextTransaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        ...
                        if (SUR.Any())
                        {
                            db.SYSUserRoles.Remove(SUR.FirstOrDefault() ?? throw new InvalidOperationException());
                            db.SaveChanges();
                        }
                        ...
                        if (SUP.Any())
                        {
                            db.SYSUserProfiles.Remove(SUP.FirstOrDefault() ?? throw new InvalidOperationException());
                            db.SaveChanges();
                        }
                        ...
                        if (SU.Any())
                        {
                            db.SYSUsers.Remove(SU.FirstOrDefault() ?? throw new InvalidOperationException());
                            db.SaveChanges();
                        }
